...
class ExposureStageComScreen extends React.Component{
    ...
    componentDidMount() {
        AsyncStorage.multiGet(['staffNumber', 'secretKey', 'projectNo', 'queryName', 'queryNumber', 'queryType']).then(res => {
            ...
            Services.getimensionality({...obj, ...{dataDimension: "exposure_stage", pageSize: 5}}).then(res => {
                if(res.isSuccess) {
            ...
            Services.getBaseInfoData({...obj, ...{dataDimension: "exposure_stage"}}).then(res => {
                if(res.isSuccess) {
    ...
    fetchList(currentPage, params){
        AsyncStorage.multiGet(['staffNumber', 'secretKey', 'projectNo', 'queryName', 'queryNumber', 'queryType']).then(res => {
            const obj = DealStorageData(res);
            const obj1 = {dataDimension: 'exposure_stage', pageSize: 5, currentPage: currentPage || 1};
...
export default ExposureStageComScreen;
