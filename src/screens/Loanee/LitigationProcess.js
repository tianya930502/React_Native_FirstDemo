...
const layoutData = {
    ...
    region: {
        isHave: true, // 是否有省份
        params: {
            dataDimension: 'lawsuit', // 'executive'为mock数据，真实数据为维度
        },
    },
...
class LitigationProcessScreen extends React.Component{
    ...
    componentDidMount() {
        AsyncStorage.multiGet(['staffNumber', 'secretKey', 'projectNo', 'queryName', 'queryNumber', 'queryType']).then(res => {
            ...
            Services.getimensionality({...obj, ...{dataDimension: "lawsuit", pageSize: 5}}).then(res => {
                if(res.isSuccess) {
            ...
            Services.getBaseInfoData({...obj, ...{dataDimension: "lawsuit"}}).then(res => {
                if(res.isSuccess) {
    ...
    fetchList(currentPage, params){
        AsyncStorage.multiGet(['staffNumber', 'secretKey', 'projectNo', 'queryName', 'queryNumber', 'queryType']).then(res => {
            const obj = DealStorageData(res);
            const obj1 = {dataDimension: 'lawsuit', pageSize: 5, currentPage: currentPage || 1};
...
export default LitigationProcessScreen;
